...
#~   select(gsim.no, KGE) %>%
#~   mutate(setup = 'uncalibrated')
rf.eval.uncalibrated <- read.csv('/scratch-shared/edwin/glorif1/original/version_1.0/output/kge_glorif1_gsim.csv') %>%
  select(gsim.no, KGE) %>%
  mutate(setup = 'uncalibrated')
...
ggsave(paste0(outputDir, 'map_KGE_glorif1.pdf'), KGE_map_uncalibrated, height = 8, width = 16, units = 'in', dpi = 1200)
