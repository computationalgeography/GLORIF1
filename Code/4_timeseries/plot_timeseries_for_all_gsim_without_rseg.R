...
#
x_min = min(merged_table$date,na.rm=T) - 365*10
...
outplott <- outplott +
 ...
 geom_text(aes(x = x_info_text, y = 0.35*y_max, label = paste("KGE GLORIF1    = ", round(kge_glorif1_gsim  , 2), sep="")), size = 2.5,hjust = 0) +
 geom_text(aes(x = x_info_text, y = 0.20*y_max, label = paste("NSE PCR-GLOBWB = ", round(nse_pcrglobwb_gsim, 2), sep="")), size = 2.5,hjust = 0) +
